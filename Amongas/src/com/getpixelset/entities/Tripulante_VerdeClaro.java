...
public class Tripulante_VerdeClaro extends Entity {
	...
	public Tripulante_VerdeClaro(int x, int y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, sprite);

		rightPlayer = Game.spritesheet.getSprite(80, 64, 16, 16);
		leftPlayer = Game.spritesheet.getSprite(80, 80, 16, 16);
		rightPlayerIdle = new BufferedImage[2];
		leftPlayerIdle = new BufferedImage[2];

		for (int i = 0; i < 2; i++) {
			rightPlayerIdle[i] = Game.spritesheet.getSprite(48 + (i * 16), 64, 16, 16);
		}
		for (int i = 0; i < 2; i++) {
			leftPlayerIdle[i] = Game.spritesheet.getSprite(48 + (i * 16), 80, 16, 16);
		}
	}
