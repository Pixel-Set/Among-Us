...
public class Tripulante_Amarelo extends Entity {
	...
	public Tripulante_Amarelo(int x, int y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, sprite);

		rightPlayer = Game.spritesheet.getSprite(32, 96, 16, 16);
		leftPlayer = Game.spritesheet.getSprite(32, 112, 16, 16);
		rightPlayerIdle = new BufferedImage[2];
		leftPlayerIdle = new BufferedImage[2];

		for (int i = 0; i < 2; i++) {
			rightPlayerIdle[i] = Game.spritesheet.getSprite(0 + (i * 16), 96, 16, 16);
		}
		for (int i = 0; i < 2; i++) {
			leftPlayerIdle[i] = Game.spritesheet.getSprite(0 + (i * 16), 112, 16, 16);
		}
	}
