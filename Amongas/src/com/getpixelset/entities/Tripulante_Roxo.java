...
public class Tripulante_Roxo extends Entity {
	...
	public Tripulante_Roxo(int x, int y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, sprite);

		rightPlayer = Game.spritesheet.getSprite(80, 128, 16, 16);
		leftPlayer = Game.spritesheet.getSprite(80, 144, 16, 16);
		rightPlayerIdle = new BufferedImage[2];
		leftPlayerIdle = new BufferedImage[2];

		for (int i = 0; i < 2; i++) {
			rightPlayerIdle[i] = Game.spritesheet.getSprite(48 + (i * 16), 128, 16, 16);
		}
		for (int i = 0; i < 2; i++) {
			leftPlayerIdle[i] = Game.spritesheet.getSprite(48 + (i * 16), 144, 16, 16);
		}
	}
