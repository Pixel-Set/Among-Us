...
public class Tripulante_Azul extends Entity {
	...
	public Tripulante_Azul(int x, int y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, sprite);

		rightPlayer = Game.spritesheet.getSprite(80, 32, 16, 16);
		leftPlayer = Game.spritesheet.getSprite(80, 48, 16, 16);
		rightPlayerIdle = new BufferedImage[2];
		leftPlayerIdle = new BufferedImage[2];

		for (int i = 0; i < 2; i++) {
			rightPlayerIdle[i] = Game.spritesheet.getSprite(48 + (i * 16), 32, 16, 16);
		}
		for (int i = 0; i < 2; i++) {
			leftPlayerIdle[i] = Game.spritesheet.getSprite(48 + (i * 16), 48, 16, 16);
		}
	}
